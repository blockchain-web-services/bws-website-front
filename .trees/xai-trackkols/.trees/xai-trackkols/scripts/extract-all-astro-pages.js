...
function extractComponents(html, pageName) {
  ...
  // Replace CDN URLs in the entire HTML
...
function createAstroPage(components, pageName) {
  ...
  const imports = [`import BaseLayout from '${pageName.includes('/') ? '../' : ''}../layouts/BaseLayout.astro';`];

  if (hasNavigation) {
    imports.push(`import Navigation from '${pageName.includes('/') ? '../' : ''}../components/Navigation.astro';`);
  }

  if (hasFooter) {
    imports.push(`import Footer from '${pageName.includes('/') ? '../' : ''}../components/Footer.astro';`);
  }

  // Check if we need Scripts component (excluding pipedrive)
  const hasScripts = scripts && scripts.includes('<script');
  if (hasScripts) {
    imports.push(`import Scripts from '${pageName.includes('/') ? '../' : ''}../components/Scripts.astro';`);
  }

  // Create the main content component
  const mainComponentName = `${pageName.replace(/[^a-zA-Z0-9]/g, '')}MainContent`;
  imports.push(`import ${mainComponentName} from '${pageName.includes('/') ? '../' : ''}../components/${mainComponentName}.astro';`);
  ...
  function findHtmlFiles(dir, basePath = '') {
    ...
    for (const file of files) {
      ...
      if (stat.isDirectory() && !file.startsWith('.') && file !== 'node_modules') {
        ...
        htmlFiles.push({ fullPath, relativePath });
      }
    }
  }
