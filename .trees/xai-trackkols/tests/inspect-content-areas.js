...
(async () => {
  ...
  console.log('\n=== Q1 Card Children ===');
  q1Elements.forEach((el, idx) => console.log(`${idx + 1}. ${el.className}: height=${el.height}px, flex=${el.flex}, marginTop=${el.marginTop}`));
  console.log(`Total: ${q1Elements.reduce((sum, el) => sum + el.height, 0)}px`);

  console.log('\n=== Q2 Card Children ===');
  q2Elements.forEach((el, idx) => console.log(`${idx + 1}. ${el.className}: height=${el.height}px, flex=${el.flex}, marginTop=${el.marginTop}`));
