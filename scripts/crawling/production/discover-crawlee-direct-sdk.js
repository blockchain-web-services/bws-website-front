/**
 * KOL Discovery using BWS X SDK - Direct Profile Fetching
 * Fetches profiles of known crypto influencers directly
 *
 * MIGRATION: Converted from discover-crawlee-direct.js to use BWS X SDK v1.6.0
 * SDK Method: client.getProfile(username)
...
async function discoverWithSDK() {
  console.log('🔍 KOL DISCOVERY - BWS X SDK MODE');
  ...
  console.log(`\n📋 Candidates to check: ${candidateUsernames.length}`);
  ...
  for (const username of testCandidates) {
    try {
      ...
      // SDK METHOD: client.getProfile() replaces getUserProfile()
  ...
  kolsData.metadata.discoveryMethod = 'sdk-direct';
  ...
  // Print results
  console.log('\n' + '='.repeat(60));
  console.log('📊 SDK DISCOVERY RESULTS');
  console.log('='.repeat(60));
  console.log(`\nExecution:`);
  ...
  try {
    await sendDiscoveryNotification({
      scriptName: 'KOL Discovery (SDK Direct)',
      success: true,
