...
`);
  ...
  if (!dryRun) {
    try {
      ...
      console.log(`✅ [${Math.round((Date.now() - scriptStartTime) / 1000)}s] Write client initialized (for anti-spam actions)\n`);
    } catch (error) {
      console.warn('⚠️  Failed to initialize write client for anti-spam actions.');
      console.warn(`   Error: ${error.message}\n`);
      console.warn('   Will skip follow/like actions but continue with replies.');
    }
  }
    ...
    try {
      ...
      for (const tweet of tweetsArray) {
        ...
        try {
          ...
          // Multi-account scraper client handles posting automatically
