...
export async function searchTweetsWebUnblocker(query, options = {}) {
  ...
  console.log(`🔍 Searching tweets (Direct + HTML): "${query}"`);
  ...
  try {
    // Launch browser WITHOUT proxy - direct access works on GitHub Actions
    console.log(`   🌐 Loading directly (no proxy)`);
    browser = await chromium.launch({
      headless: true,
      args: [
        '--disable-blink-features=AutomationControlled',
    ...
    try {
      ...
      console.log(`   ⚠️  No tweets found immediately, waiting longer...`);
